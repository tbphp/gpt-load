...
**升级提示：此前版本始终中继。升级到支持此设置的版本后，未设置模式时使用直连；依赖服务器传输音频的部署请显式选择“网关中继”。** 语音模型仍无需加入分组，客户端指定的模型原样传给上游。客户端必须建立控制 WebSocket；直连会话创建后 30 秒未接入控制连接，或控制断线后 30 秒未重连，会清理并挂断会话。上游挂断失败时接口返回 502，会话保留为待结束状态并占用并发名额，服务器每 5 秒重试，也可再次请求挂断；待结束会话不能重新接入控制连接。确认结束后只写入一次最终日志，停机时仍无法确认则记为“未完成”；usage/cost 仍可能不可用。
