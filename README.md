...
**Upgrade: previous versions always relayed media. The new unset default is Direct; deployments relying on server media transport must explicitly select Gateway relay.** No group voice-model setup is required; the client's model is forwarded unchanged. Clients must attach a control WebSocket within 30 seconds and reconnect within 30 seconds after disconnection. Failed upstream hangups return HTTP 502 and retain the terminating session and its capacity slot. The server retries every 5 seconds; the owner can also retry hangup, but control reattachment is rejected. One final log is emitted after confirmation, or as incomplete if shutdown cannot confirm termination. Usage/cost may remain unavailable.
