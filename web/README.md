...
路由加载失败时提供刷新重试。分组工作区采用可展开卡片，展示服务原因、配置摘要、模型预览与最近活跃小时；支持本地搜索、渠道与运行视图筛选、排序、复制地址、就地查看模型、启停和右侧基础编辑。完整轻量快照一次读取，超过 60 个卡片时分批追加展示，不使用固定页码。未实现的总览、其他业务页面、独立分组详情和导入页面只保留标题，内容区留空。后续业务页面继续遵守 principal 权限合同。
