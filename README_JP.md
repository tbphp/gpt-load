...
CC Switch の「使用量照会」でカスタムスクリプトを有効にし、[照会スクリプト](examples/cc-switch-usage.js)を貼り付けます。GPT-Load の URL（`/v1` の有無は任意）と AccessKey を使用し、管理キーは不要です。

`GET /v1/usage` は AccessKey で認証し、そのキー自身の `isValid`、`total`、`used`、`remaining` を常に返します。金額は USD の推定コストです。総額上限がある場合は、そのルールの上限と累積使用量（既存の手動リセットを含む）を使用し、残額の下限はゼロです。周期上限は計算に含めません。総額上限がない場合、`total` と `remaining` は `0`、`used` は長期保存された累積統計（自動モデル選択の判断コストを含む）を使用します。非同期保存による遅延があり、この場合スクリプトは使用量のみ表示します。

認証済みの照会成功時は、上限到達後も `isValid: true` を返します。無効・無効化済み・期限切れ・接続元 IP 制限に違反するキーは認証エラー、データ取得失敗はエラーを返します。照会は推論の使用量上限や RPM 制限で遮断されず、それらを消費しません。
