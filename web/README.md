...
浏览器偏好为 `gpt-load.frontend`，有效值为 `classic`、`modern`，默认 `modern`。选择保存后重新加载 `/settings`，每次只创建一个应用和路由实例。语言、明暗模式保留既有浏览器存储约定。经典版只读取 localStorage 登录凭据；新版未勾选“记住登录”时，切换经典版需要重新登录。
...
路由加载失败时提供刷新重试。分组工作区采用双行列表：分组与渠道、凭据总数及状态进度条、模型数量及计价倍率、近 24 小时请求与用量、启停与行内权重。支持本地搜索、渠道与运行视图筛选、排序、复制地址、就地展开完整模型及右侧基础编辑。完整轻量快照一次读取，默认每页 20 条，可选 50 / 100；表头与底部分页固定，仅内容区滚动。未实现的总览、其他业务页面、独立分组详情和导入页面只保留标题，内容区留空。后续业务页面继续遵守 principal 权限合同。
...
侧栏底部以两行两列展示外链：首行为使用文档、赞助支持，次行为 GitHub、Telegram。GitHub 使用官方 Octicons 标识。分隔线下版本号居中，管理员更新检查按钮紧邻版本号。版本来自现有 `/health`，新版本只在用户点击时通过 `/api/system/update` 检查，传 `force=true` 并使用当前已验证的管理员会话。状态由新版布局统一持有，桌面和移动侧栏共享结果，不重复检查；访问密钥用户不查询管理员更新接口。

新版页面可以独立设计展示 DTO、聚合查询及管理接口。`modern/api/` 为其独立接口代码目录。分组展示使用 `/api/modern/groups`，当前页用量通过 `/api/modern/groups/usage` 批量读取，每次最多 100 个分组，复用后端精确时间窗口及最终请求归属。边界明细查询使用分组与完成时间联合索引，避免逐组扫描或计算完整排行榜。启停与编辑继续调用既有管理接口，旧接口合同及核心流程、数据面行为保持稳定。
...
HTTP 客户端按请求捕获会话版本；身份已切换或请求已取消时，迟到的 401 不清除新会话。会话恢复及登录响应也检查版本和取消状态，防止退出后被旧响应重新登录。所选存储不可用时仍使用内存会话，不记住时不会回退到 localStorage；业务代码和版本检查从会话客户端取凭据，不直接读取浏览器存储。认证逻辑和组件不依赖 classic，不修改经典版、共享 HTTP 实现或后端。
