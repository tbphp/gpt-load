  ...
  group: {
    ...
    settings: {
      ...
      parameterOverrides: {
        ...
        orderHelp: '一致するルールは上から実行され、後のルールが優先されます。',
        addRule: 'ルールを追加',
        ...
        conditions: '一致条件',
        protocol: 'プロトコル',
        protocolHelp: '未指定の場合は全プロトコルに一致します。',
        model: 'モデル',
        modelHelp: '完全一致、または末尾の * でモデルの前方一致を指定します。',
        modelNoMatch: '現在一致するグループモデルはありませんが、保存できます。',
        allProtocolWarning:
        ...
        set: 'パラメーターを設定',
        setHelp:
          'JSON オブジェクトを入力します。オブジェクトは再帰的に結合し、配列とスカラーは置換します。変数は展開されません。',
        setPlaceholder: "{'{'}\n  \"temperature\": 0.7\n{'}'}",
        formatJSON: 'JSON を整形',
        remove: 'パラメーターを削除',
        removeHelp:
          '/ で始まるオブジェクトフィールドのパスを使用します。配列インデックスは未対応です。',
        addRemove: 'パスを追加',
        removePath: '削除パス',
