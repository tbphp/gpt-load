...
test('critical management journey works through the embedded binary', async ({ page }) => {
  ...
  await test.step(
    ...
    async () => {
      ...
      await page.goto('/')
      await expect(page.locator('[data-test="home-success-rate"]')).toContainText(
        `${expectedUsageRequests} requests`,
      )
      await expect(page.locator('[data-test="home-estimated-cost"]')).toContainText(
        '$0.00 + unknown',
      )
      await expect(page.locator('[data-test="home-estimated-cost"]')).toContainText(
        `${expectedUsageRequests} unpriced`,
      )
      await expect(page.locator('[data-test="home-cost-ranking"]')).toBeVisible()
      await expect(page.locator('[data-test="home-ranking-footer"] a')).toHaveAttribute(
        'href',
  ...
  await test.step('verify responsive light and dark layouts with table-local overflow', async () => {
    for (const locale of visualLocales) {
      ...
      for (const theme of visualThemes) {
        ...
        for (const viewport of visualViewports) {
          ...
          await expect(page.locator('[data-test="home-connection-placeholder"]')).toBeVisible()

          const layout = await page.evaluate(() => {
            const topbar = document.querySelector('.app-topbar')?.getBoundingClientRect()
            const main = document.querySelector('#main-content')?.getBoundingClientRect()
            const lede = document.querySelector('[data-test="home-lede"]')?.getBoundingClientRect()
            const usage = document
              .querySelector('[data-test="home-usage"]')
              ?.getBoundingClientRect()
            const ranking = document
              .querySelector('[data-test="home-cost-ranking"]')
              ?.getBoundingClientRect()
            const connection = document
              .querySelector('[data-test="home-connection-placeholder"]')
              ?.getBoundingClientRect()
            return {
              ...
              rankingBottom: ranking?.bottom ?? -1,
              rankingTop: ranking?.top ?? -1,
          ...
          expect(layout.ledeTop).toBeGreaterThanOrEqual(layout.mainTop)
          expect(layout.usageTop).toBeGreaterThanOrEqual(layout.ledeBottom)
          expect(layout.rankingTop).toBeGreaterThanOrEqual(layout.usageTop)
          expect(layout.rankingBottom).toBeLessThanOrEqual(layout.usageBottom)
          expect(layout.connectionTop).toBeGreaterThanOrEqual(layout.usageBottom)
        }
      }
    }
    ...
    const connectionPlaceholder = page.locator('[data-test="home-connection-placeholder"]')
    await expect(connectionPlaceholder).toContainText('Placeholder')
    await expect(
      connectionPlaceholder.locator(
        'button, a, input, select, textarea, code, [role="button"], [aria-expanded]',
      ),
    ).toHaveCount(0)
