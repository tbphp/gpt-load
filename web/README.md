...
浏览器界面偏好使用 `gpt-load.frontend.v2`：选择经典版时保存 `classic`，选择新版时清除该值，默认 `modern`。旧键 `gpt-load.frontend` 在启动时清理，不再参与界面选择。新版顶栏的显示模式菜单与经典版顶栏菜单均提供「新版 / 经典版」切换，对所有人开放，包括登录页和访问密钥用户。顶栏切换后重新加载首页，登录页切换则留在登录页；设置页原有入口仍返回 `/settings`。每次只创建一个应用和路由实例。

启动选择器直接读取浏览器界面偏好，不查询认证接口。刷新、退出登录和认证失效均保留选择；存储读取不可用或偏好值无效时默认使用新版。选择界面不改变身份、路由和接口权限，两套界面各自验证会话并限制管理员页面。语言和明暗模式保留既有浏览器存储约定。
