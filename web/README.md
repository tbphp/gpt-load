...
侧栏底部先展示使用文档，再并排展示 GitHub 与 Telegram；分隔线下显示真实版本和更新检查。版本来自现有 `/health`，更新检查复用 `/api/system/update` 和浏览器已有的管理凭据；自动查询使用后端缓存，手动检查传 `force=true`，无管理权限时明确提示。状态由新版布局统一持有，桌面和移动侧栏共享结果，不重复检查，不修改既有登录凭据。
