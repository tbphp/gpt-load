...
**更新時の注意：従来は常に中継でした。新バージョンの未設定時は直接接続になります。サーバー経由の音声が必要な場合はゲートウェイ中継を明示的に選択してください。** グループへの音声モデル登録は不要で、クライアント指定モデルをそのまま送信します。作成後 30 秒以内に制御 WebSocket を接続し、切断後も 30 秒以内に再接続する必要があります。正常終了を確認できない場合や上流切断に失敗した場合は未完了として記録します。使用量・コストが取得できない場合もあります。
