...
快速跳转按页面名称、分类和路径查找入口，支持键盘操作；路由加载失败时提供刷新重试。尚未实现的总览、业务页面、分组详情、导入和登录页面只保留标题，内容区留空。当前提供界面偏好和版本检查，尚未接入业务操作或登录流程，不显示虚构的统计、在线状态或身份。后续接入业务数据时继续遵守既有管理认证与 principal 权限合同。

本地界面设置不读写服务端配置，也不需要管理权限。侧栏收起状态使用 `gpt-load.modern.sidebar-collapsed`；明暗模式和语言延续已有偏好键。新版设置中的现代界面缩略图使用当前框架截图；旧版保留既有的切换组件和示意缩略图。

侧栏底部以两行两列展示外链：首行为使用文档、赞助支持，次行为 GitHub、Telegram。GitHub 使用官方 Octicons 标识。分隔线下显示真实版本和更新检查。版本来自现有 `/health`，更新检查复用 `/api/system/update` 和浏览器已有的管理凭据；自动查询使用后端缓存，手动检查传 `force=true`，无管理权限时明确提示。状态由新版布局统一持有，桌面和移动侧栏共享结果，不重复检查，不修改既有登录凭据。
