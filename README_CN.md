...
**升级提示：此前版本始终中继。升级到支持此设置的版本后，未设置模式时使用直连；依赖服务器传输音频的部署请显式选择“网关中继”。** 语音模型仍无需加入分组，客户端指定的模型原样传给上游。客户端必须建立控制 WebSocket；直连会话创建后 30 秒未接入控制连接，或控制断线后 30 秒未重连，会清理并挂断会话。未确认正常结束或上游挂断失败时日志记为“未完成”；usage/cost 仍可能不可用。
