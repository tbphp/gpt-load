...
func TestCredentialCombinedConcurrencySaveIsAtomic(t *testing.T) {
	fixture := newServiceFixture(t)
	groupID := createGroupWithCredentials(t, fixture, "sk-credential-combined")
	var original models.Credential
	if err := fixture.db.Where("group_id = ?", groupID).Take(&original).Error; err != nil {
		t.Fatal(err)
	}
	request := CredentialUpdateRequest{
		...
		MaxConcurrency: optionalField[int64]{Set: true, Value: 4},
	}
	before := fixture.manager.Current()
	invalid := request
	invalid.MaxConcurrency.Value = -1
	if _, err := fixture.service.UpdateGroupCredential(t.Context(), groupID, original.ID, invalid); !errors.Is(err, app_errors.ErrValidation) {
		t.Fatalf("invalid combined save: %v", err)
	}
	if err := fixture.db.Exec(`CREATE TRIGGER reject_credential_concurrency
		BEFORE INSERT ON concurrency_policies BEGIN SELECT RAISE(ABORT, 'forced rollback'); END`).Error; err != nil {
		t.Fatal(err)
	}
	fixture.service.db = fixture.db.Session(&gorm.Session{Logger: logger.Default.LogMode(logger.Silent)})
	if _, err := fixture.service.UpdateGroupCredential(t.Context(), groupID, original.ID, request); !errors.Is(err, app_errors.ErrDatabase) {
		t.Fatalf("failed combined save: %v", err)
	}
	var saved models.Credential
	if err := fixture.db.First(&saved, original.ID).Error; err != nil {
		t.Fatal(err)
	}
	if saved.UpdatedAtMS != original.UpdatedAtMS || !equalOptionalWeight(saved.WeightManual, original.WeightManual) ||
		saved.ProxyConfig != nil || fixture.manager.Current() != before {
		t.Fatal("failed save changed credential metadata or published a snapshot")
	}
	view, ok := findRuntimeCredential(fixture.registry.Snapshot(), original.ID)
	if !ok || !equalOptionalWeight(view.WeightManual, original.WeightManual) {
		t.Fatal("failed save changed runtime weight")
	}
	assertConcurrencyPoliciesAbsent(t, fixture, concurrency.Credential(original.ID))
	if err := fixture.db.Exec("DROP TRIGGER reject_credential_concurrency").Error; err != nil {
		t.Fatal(err)
	}
	// Retry the same payload after a failed write; metadata and policy commit once.
	if _, err := fixture.service.UpdateGroupCredential(t.Context(), groupID, original.ID, request); err != nil {
		t.Fatal(err)
	}
	if err := fixture.db.First(&saved, original.ID).Error; err != nil {
		t.Fatal(err)
	}
	if saved.WeightManual == nil || *saved.WeightManual != 7 || saved.ProxyConfig == nil ||
		fixture.manager.Current().Revision != before.Revision+1 ||
		fixture.manager.Current().ConcurrencyPolicies[concurrency.Credential(original.ID)] != 4 {
		t.Fatal("retry did not commit metadata and concurrency together")
	}
	for _, maximum := range []optionalField[int64]{{Set: true, Value: 0}, {Set: true, Null: true}} {
		if _, err := fixture.service.UpdateGroupCredential(t.Context(), groupID, original.ID, CredentialUpdateRequest{MaxConcurrency: maximum}); err != nil {
			t.Fatal(err)
		}
		limit, exists := fixture.manager.Current().ConcurrencyPolicies[concurrency.Credential(original.ID)]
		if exists == maximum.Null || limit != 0 {
			t.Fatalf("unlimited/inheritance: limit=%d exists=%t", limit, exists)
		}
	}
}

func TestCredentialCombinedSaveSharesOnlyAccountConcurrency(t *testing.T) {
	fixture := newServiceFixture(t)
	groupID, one := createHomeSubscriptionCredential(t, fixture, "combined-one", "combined-account", "combined@example.com")
	_, two := createHomeSubscriptionCredential(t, fixture, "combined-two", "combined-account", "combined@example.com")
	_, other := createHomeSubscriptionCredential(t, fixture, "combined-other", "other-account", "other@example.com")
	before := fixture.manager.Current()
	if _, err := fixture.service.UpdateGroupCredential(t.Context(), groupID, one.ID, CredentialUpdateRequest{
		WeightManual:   optionalField[int]{Set: true, Value: 9},
		MaxConcurrency: optionalField[int64]{Set: true, Value: 2},
	}); err != nil {
		t.Fatal(err)
	}
	views, err := fixture.service.readConcurrency([]concurrencyTarget{{"credential", one.ID}, {"credential", two.ID}, {"credential", other.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if fixture.manager.Current().Revision != before.Revision+1 ||
		views.Items[0].EffectiveMaxConcurrency != 2 || views.Items[1].EffectiveMaxConcurrency != 2 ||
		!views.Items[1].Shared || views.Items[2].MaxConcurrency != nil {
		t.Fatalf("account limit scope: %+v", views.Items)
	}
	var unchanged models.Credential
	if err := fixture.db.First(&unchanged, two.ID).Error; err != nil || !equalOptionalWeight(unchanged.WeightManual, two.WeightManual) {
		t.Fatalf("other group's weight changed: %+v, %v", unchanged.WeightManual, err)
	}
	if _, err := fixture.service.UpdateGroupCredential(t.Context(), groupID, one.ID, CredentialUpdateRequest{
		WeightManual: optionalField[int]{Set: true, Value: 8},
	}); err != nil {
		t.Fatal(err)
	}
	views, err = fixture.service.readConcurrency([]concurrencyTarget{{"credential", two.ID}})
	if err != nil || views.Items[0].EffectiveMaxConcurrency != 2 {
		t.Fatalf("metadata-only edit lost account override: %+v, %v", views.Items, err)
	}
}
...
func TestConcurrencyHTTPAuthorizationAndValidation(t *testing.T) {
	initControlI18n(t)
	fixture := newServiceFixture(t)
	groupID := createGroupWithCredentials(t, fixture, "sk-concurrency-http")
	var credential models.Credential
	if err := fixture.db.Where("group_id = ?", groupID).Take(&credential).Error; err != nil {
		t.Fatal(err)
	}
	credentialPath := fmt.Sprintf("/api/groups/%d/credentials/%d", groupID, credential.ID)
	...
	for _, tc := range []struct {
		...
		{"PUT", credentialPath, `{"weight_manual":5,"max_concurrency":2}`, key.Key, 403},
		{"PUT", credentialPath, `{"weight_manual":5,"max_concurrency":1.5}`, authTestKey, 400},
		{"PUT", credentialPath, `{"weight_manual":5,"max_concurrency":1000001}`, authTestKey, 400},
		{"PUT", credentialPath, `{"weight_manual":5,"max_concurrency":2}`, authTestKey, 200},
		{"PUT", credentialPath, `{"max_concurrency":0}`, authTestKey, 200},
		{"PUT", credentialPath, `{"max_concurrency":null}`, authTestKey, 200},
