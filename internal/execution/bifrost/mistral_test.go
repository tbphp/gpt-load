...
func TestMistralNativeRoutesPreserveMethodPathAndCredential(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "batch job", operation: execution.OperationMistralBatch, method: http.MethodGet, path: "/v1/batch/jobs",
			wantPath: "/v1/batch/jobs",
		},
		{
			name: "file content", operation: execution.OperationMistralFiles, method: http.MethodGet, path: "/v1/files/file-1/content",
			wantPath: "/v1/files/file-1/content",
		},
		{
			name: "voices", operation: execution.OperationMistralVoices, method: http.MethodGet, path: "/v1/audio/voices",
			wantPath: "/v1/audio/voices",
		},
		{
			name: "voices v2", operation: execution.OperationMistralVoices, method: http.MethodGet, path: "/v2/audio/voices",
