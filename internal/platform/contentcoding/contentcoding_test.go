...
func TestParseContentEncodingRejectsUnsupportedOrStackedValues(t *testing.T) {
	for _, values := range [][]string{{"compress"}, {"gzip, br"}, {"gzip", "br"}} {
...
func TestReadDecodedBodySupportsAllSingleEncodings(t *testing.T) {
	...
	for _, encoding := range []Encoding{EncodingIdentity, EncodingGzip, EncodingBrotli, EncodingDeflate, EncodingZstd} {
...
func TestReadDecodedBodyRejectsMalformedEncodedData(t *testing.T) {
	for _, encoding := range []Encoding{EncodingGzip, EncodingBrotli, EncodingDeflate, EncodingZstd} {
		t.Run(string(encoding), func(t *testing.T) {
