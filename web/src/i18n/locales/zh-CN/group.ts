export default {
  ...
  group: {
    ...
    settings: {
      ...
      parameterOverrides: {
        description:
          '按顺序修改此分组的生成与向量请求；不影响模型列表、Token 统计、Responses 资源操作或系统探测。配置以明文保存，请勿填写凭据。',
        orderHelp: '所有命中规则从上到下依次执行；越靠后的规则优先级越高。',
        addRule: '添加规则',
        emptyTitle: '当前不修改请求参数',
        emptyDescription: '添加规则后，只修改命中条件的业务推理请求。',
        rule: '规则 {position}',
        summary: '设置 {set} · 删除 {remove}',
        ...
        conditions: '生效条件（可选）',
        protocol: '请求协议',
        protocolHelp: '留空表示全部协议；规则值始终按客户端请求协议填写。',
        model: '请求模型',
        modelHelp: '匹配客户端可见模型；支持精确值或末尾 * 前缀匹配。',
        modelNoMatch: '当前分组没有匹配的对外模型；仍可保存供后续模型使用。',
        allProtocolWarning: '此规则会作用于全部支持的协议，请确认参数名称在各协议中兼容。',
        allProtocols: '全部协议',
        allModels: '全部模型',
        set: '设置参数',
        setHelp:
          "填写需要添加或覆盖的 JSON 对象；对象递归合并，数组和标量整体替换。暂不支持变量，{'$'}{'{'}...{'}'} 会作为普通字符串发送。",
        setPlaceholder: "{'{'}\n  \"temperature\": 0.7\n{'}'}",
        formatJSON: '格式化 JSON',
        remove: '删除参数',
        removeHelp: '填写以 / 开头的对象字段路径；数组请整体删除或替换。',
        addRemove: '添加删除路径',
        removePath: '删除参数路径',
        deleteRemove: '移除此删除路径',
        protocols: {
        ...
        errors: {
          ...
          removeDuplicate: '删除路径不能重复。',
          actionRequired: '至少设置一个参数或添加一条删除路径。',
        },
      },
