...
func TestRPMQueriesKeepPeaksAndMissingKeysSeparate(t *testing.T) {
	...
	report, err := s.QueryRPM(t.Context(), rpm.AccessKey, 1, now.Add(-7*24*time.Hour), now)
	if err != nil || report.Peak == nil || *report.Peak != 100 || report.BucketWidthMS != 3_600_000 || len(report.Points) != 2 {
		t.Fatalf("report=%+v error=%v", report, err)
	}
	missing, err := s.QueryRPM(t.Context(), rpm.AccessKey, 3, now.Add(-time.Hour), now)
