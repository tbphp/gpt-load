export default {
  ...
  group: {
    ...
    settings: {
      ...
      parameterOverrides: {
        description:
          'このグループの生成・埋め込みリクエストを順番に変更します。モデル一覧、トークン計算、Responses リソース操作、プローブには影響しません。値は平文で保存されるため、認証情報は入力しないでください。',
        orderHelp: '一致するルールは上から順に実行され、後のルールが優先されます。',
        addRule: 'ルールを追加',
        emptyTitle: 'リクエストパラメーターは変更されません',
        emptyDescription: 'ルールを追加すると、一致する推論リクエストだけを変更します。',
        rule: 'ルール {position}',
        summary: '設定 {set} · 削除 {remove}',
        ...
        conditions: '適用条件（任意）',
        protocol: 'リクエストプロトコル',
        protocolHelp: '未指定は全プロトコルです。値はクライアント側の形式で入力します。',
        model: 'リクエストモデル',
        modelHelp: '公開モデル名の完全一致、または末尾 * の前方一致を使用できます。',
        modelNoMatch: '現在一致する公開モデルはありませんが、将来のモデル用に保存できます。',
        allProtocolWarning:
          'このルールは全プロトコルに適用されます。パラメーター名の互換性を確認してください。',
        allProtocols: '全プロトコル',
        allModels: '全モデル',
        set: 'パラメーターを設定',
        setHelp:
          "追加・上書きする JSON オブジェクトを入力します。オブジェクトは再帰的に結合し、配列とスカラーは置換します。変数は未対応で、{'$'}{'{'}...{'}'} は通常の文字列として送信されます。",
        setPlaceholder: "{'{'}\n  \"temperature\": 0.7\n{'}'}",
        formatJSON: 'JSON を整形',
        remove: 'パラメーターを削除',
        removeHelp:
          '/ で始まるオブジェクトフィールドのパスを入力します。配列は全体を削除または置換します。',
        addRemove: '削除パスを追加',
        removePath: 'パラメーター削除パス',
        deleteRemove: '削除パスを削除',
        protocols: {
        ...
        errors: {
          ...
          removePointer:
            'パスは / で始め、有効な ~0、~1 エスケープを使用し、配列インデックスを含めないでください。',
          removeDuplicate: '削除パスは重複できません。',
          actionRequired: '少なくとも 1 つの設定または削除パスを追加してください。',
        },
      },
