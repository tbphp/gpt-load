...
Embeddings には 2 つの独立したネイティブプロトコルがあります。`openai-embeddings`（`POST /v1/embeddings`）は OpenAI、OpenRouter、OpenAI Compatible、New API、GPT-Load の API Key チャネルで、`gemini-embeddings`（`POST /v1beta/models/{model}:embedContent` と `:batchEmbedContents`）は Gemini、New API、GPT-Load の API Key チャネルで利用できます。リクエストとレスポンスは各プロバイダーのネイティブ形式のままです。Gemini チャネルはテキスト入力を `:batchEmbedContents` に変換することで `openai-embeddings` も受け付けます。`dimensions` と `float` / `base64` 出力に対応し、Token ID 入力には対応していません。1 リクエストあたりの件数上限は Gemini 側で検証されます。同じクライアントモデル名の背後にあるグループには同じ Embedding モデルを使用してください。異なるモデルのベクトルが混在するのを防ぐためです。サブスクリプションチャネルには対応していません。プロトコルフィルターを設定していない AccessKey は既存の「有効なプロトコルをすべて許可する」動作を維持するため、アップグレード後に両方の Embeddings プロトコルへのアクセス権も得ます。最小権限で運用する場合は、プロトコルフィルターを明示的に設定してください。
...
**システム設定 → 接続とタイムアウト → リアルタイム音声** で既定値を選択します。各 Codex グループの **詳細設定 / 実行時設定** では継承、**無効、上流に直接接続、ゲートウェイ中継** を選べます。異なるモードのグループを併用でき、通話は選択されたグループとアカウントに固定されます。無効にすると対象の音声通話も終了しますが、テキスト機能は維持されます。

- **上流に直接接続（既定）**：GPT-Load が認証、アカウント選択、通話作成、制御 WebSocket の中継を担当します。音声と WebRTC データチャネルはクライアントから上流へ直接接続します。アカウントトークンはサーバーに保持されます。サーバーのメディア UDP 公開は不要ですが、クライアントから上流への到達性が必要です。サーバーの送信プロキシはこのクライアント通信には適用されません。
- **ゲートウェイ中継**：音声とデータチャネルは GPT-Load を経由し、設定済みの上流プロキシを利用します。全中継グループでメディア IP と UDP 範囲を共有します。
- **アカウントと再試行**：既存のグローバル加重スケジューリングと追加再試行回数（既定 2）を利用します。明確な 403/429 拒否時は次の候補へ、401 は必要に応じて認証情報を更新し、同じ予算内で再試行します。タイムアウト、結果不明の 5xx、通話作成後の切断では別アカウントに再作成しません。音声の利用拒否でテキスト機能をブラックリストに入れません。音声非対応と判明しているグループは無効にしてください。プラン名から権限を推測しません。

**更新時の注意：従来は常に中継でした。新バージョンの未設定時は直接接続になります。サーバー経由の音声が必要な場合はゲートウェイ中継を明示的に選択してください。** グループへの音声モデル登録は不要で、クライアント指定モデルをそのまま送信します。両モードとも作成後 30 秒以内に制御 WebSocket を接続し、切断後も 30 秒以内に再接続する必要があります。作成応答の Location に `/hangup` を追加すると明示的に切断できます。ローカル接続を閉じ、上流への切断を一度試みた時点で通話ログを一件記録します。初回の上流切断に失敗すると HTTP 502 を返し、バックグラウンド処理を待たずに未完了として記録します。切断は 5 秒ごとに再試行し、初回と手動再試行を含め最大 3 回です。再試行中は同時接続枠を保持して制御再接続を拒否し、成功または上限到達時にローカルの枠を解放します。後の再試行で通話ログを変更・重複記録しません。上限到達は上流の終了確認を意味しません。警告ログにはエラー種別と取得できた HTTP ステータスを記録します。使用量・コストが取得できない場合もあります。

Docker 中継では基本 Compose と同じバージョンの [`docker-compose.voice.yml`](docker-compose.voice.yml) を取得し、`.env` にクライアントから到達できる公開メディア IP を設定します。起動後にシステムまたはグループで中継を選択してください。

```dotenv
CODEX_LIVE_PUBLIC_IP=サーバーの公開IP
```
...
待受と Docker 公開範囲は同じ変数で設定され、既定は **UDP 50000–50127** です。クラウドのセキュリティグループとホストのファイアウォールでも同じ範囲を許可してください。変更時は `.env` の `CODEX_LIVE_UDP_PORT_MIN` と `CODEX_LIVE_UDP_PORT_MAX` を両方設定し、コンテナを再作成します。Nginx は WebSocket Upgrade と長時間接続のタイムアウト設定が必要ですが、通常の HTTP プロキシはメディア UDP を中継しません。別ホスト、NAT、CDN 環境ではメディア IP/ポートへの到達性や `CODEX_LIVE_ICE_SERVERS` の STUN/TURN 設定が必要です。公開 IP や外部ファイアウォールは自動設定できません。
