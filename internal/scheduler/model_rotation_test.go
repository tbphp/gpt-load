...
func TestModelRotationSkipsCooldownWithoutLosingOtherModels(t *testing.T) {
	...
	if got := *modelRotationPick(t, snapshot, registry, query).UpstreamModelID; got != "m1" {
		t.Fatalf("recovery tried to catch up: %s", got)
	}
}
...
func TestModelRotationContinuesAfterModelRemovalAndCleansDeletedGroups(t *testing.T) {
	snapshot, registry := modelRotationFixture(t)
	query := modelRotationQuery(1)
	modelRotationPick(t, snapshot, registry, query)
	modelRotationPick(t, snapshot, registry, query)
	...
	snapshot.Revision++
	if got := *modelRotationPick(t, snapshot, registry, query).UpstreamModelID; got != "m3" {
		t.Fatalf("removing the previous model reset the cursor: %s", got)
	}
