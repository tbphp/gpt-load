...
Embeddings 在 OpenAI、OpenRouter 和 OpenAI Compatible API Key 渠道提供原生 OpenAI-compatible Wire，并在 Google Gemini 渠道支持转换为 `batchEmbedContents`（单次请求上限 100 条文本，不支持 Token ID 输入）。为避免向量空间被污染，操作员应确保轮换或故障回退候选组保持相同的 Embedding 语义空间。不支持订阅渠道。未设置协议过滤器的 AccessKey 会按既有语义允许全部已启用协议，升级后也会获得 Embeddings 访问能力；最小权限部署请显式配置协议过滤器。
