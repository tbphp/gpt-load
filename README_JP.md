...
**更新時の注意：従来は常に中継でした。新バージョンの未設定時は直接接続になります。サーバー経由の音声が必要な場合はゲートウェイ中継を明示的に選択してください。** グループへの音声モデル登録は不要で、クライアント指定モデルをそのまま送信します。作成後 30 秒以内に制御 WebSocket を接続し、切断後も 30 秒以内に再接続する必要があります。上流切断に失敗すると HTTP 502 を返し、終了待ちの通話と同時接続枠を保持します。サーバーは 5 秒ごとに再試行し、利用者も切断を再試行できますが、制御接続の再接続は拒否します。確認後に最終ログを一度だけ記録し、停止時も終了を確認できない場合は未完了とします。使用量・コストが取得できない場合もあります。
