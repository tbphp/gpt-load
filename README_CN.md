...
在「设置 → 并发限制」配置全局请求上限，以及分组、AccessKey、账号/凭据的默认上限；各分组设置、AccessKey 编辑页和凭据详情可单独覆盖。选择继承使用对应系统默认值，`0` 表示不限，正整数上限为 1,000,000。初始默认均不限；设置、分组和 AccessKey 的并发修改使用页面主保存按钮统一应用，凭据详情中的修改使用编辑器内的保存按钮。
