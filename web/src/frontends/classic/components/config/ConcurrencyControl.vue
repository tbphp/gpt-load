...
const dirty = computed(
  () => baseline.value !== undefined && (!valid.value || selectedMaximum.value !== baseline.value),
)
...
function initializeEditor(maximum = query.data.value?.max_concurrency): void {
...
function edit(): void {
  if (baseline.value === undefined || !dirty.value) initializeEditor()
  failed.value = false
  saved.value = false
  restoringDefault.value = false
  editing.value = true
...
async function persist(ignoreDisabled: boolean): Promise<boolean> {
  ...
  try {
    await updateConcurrency(client, props.scope, props.id, maximum)
    synchronize(maximum)
    editing.value = false
    saved.value = true
    restoringDefault.value = false
...
function synchronize(maximum: number | null): void {
  const queryKey = [...concurrencyQueryKey, props.scope, props.id]
  void queryClient.cancelQueries({ queryKey, exact: true })
  queryClient.setQueryData<ConcurrencyView>(queryKey, (data) =>
    data
      ? {
          ...data,
          ...
          effective_max_concurrency:
            maximum ??
            (props.scope === 'global' || isDefault.value ? 0 : data.effective_max_concurrency),
          source: maximum === null ? 'default' : 'override',
        }
      : undefined,
  )
  initializeEditor(maximum)
  editing.value = false
  failed.value = false
  restoringDefault.value = false
  // The update succeeded; a failed refresh must not discard the saved draft.
  void queryClient.invalidateQueries({ queryKey: concurrencyQueryKey })
}
function reset(): void {
  initializeEditor()
  editing.value = false
    ...
    >
      ...
      >
        <AppSelect
          ...
          :options="options"
          :disabled="pending || disabled || baseline === undefined"
          size="sm"
        />
        <AppTooltip
