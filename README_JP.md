...
「設定 → 同時実行数の制限」で全体のリクエスト上限と、Group・AccessKey・アカウント/認証情報の既定上限を設定できます。各設定画面で個別に上書きできます。継承は対応するシステム既定値、`0` は無制限、正整数は最大 1,000,000 です。初期状態はすべて無制限です。設定・Group・AccessKey の変更は各画面のメイン保存ボタンでまとめて適用し、認証情報の詳細ではエディター内の保存ボタンを使用します。
