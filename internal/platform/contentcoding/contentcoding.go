...
// DecodeBytesLimited decodes one representation while bounding the decoded
// output and decoder memory/window use where the codec supports it.
func DecodeBytesLimited(encoding Encoding, wire []byte, limit int64) ([]byte, error) {
	...
	case EncodingZstd:
		...
		reader = decoded
		closeReader = func() error { decoded.Close(); return nil }
	default:
...
// preference syntax is treated compatibly: only a valid, explicit rejection
// is allowed to reject an otherwise usable plaintext response.
