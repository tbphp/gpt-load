export default {
  monitor: {
    title: 'モニター',
    description: 'ランタイムの健全性、リクエストログ、現在のルート判断を確認します。',
    help: '説明を表示',
    currentState: '現在の状態',
    tabs: {
      label: 'モニターのセクション',
      health: 'ヘルス',
      logs: 'リクエストログ',
      usage: '使用量とコスト',
      inspector: 'ルート検査',
    },
    health: {
      refresh: '更新',
      loading: 'ランタイムヘルスを読み込み中…',
      loadFailed: 'ランタイムヘルスを読み込めません。',
      stale: 'ヘルスデータが古い可能性があります。更新に失敗し、回復タイマーは停止しています。',
      none: 'なし',
      overview: {
        label: 'ヘルス概要',
        available: '利用可能なキー',
        total: '全 {total} キー',
        cooldown: 'クールダウン中',
        cooldownClear: '回復待ちはありません',
        cooldownRecovery: '最短 {time} 後に回復',
        blacklisted: 'ブラックリスト',
        blacklistedRecovery: '各キーの回復案内を確認してください',
        blacklistedClear: 'ブラックリストのキーはありません',
        disabled: '設定により除外',
        disabledDescription: '設定によりスケジュール対象外',
      },
      status: {
        empty: 'アップストリーム Key がありません',
        unavailable: '現在利用可能なアップストリーム Key がありません',
        exceptions: 'ランタイム例外があります',
        clear: '現在アップストリーム Key のランタイム例外はありません',
      },
      summary: {
        title: 'ランタイムヘルス',
        description:
          '現在のアップストリーム Key ランタイムスナップショットが公開する状態と生のカウンターです。',
        revision: 'リビジョン {revision}',
        observedAt: '観測時刻 {time}',
      },
      counts: {
        total: '合計 {total}',
        available: '利用可能 {available}',
        cooldown: 'クールダウン {cooldown}',
        blacklisted: 'ブラックリスト {blacklisted}',
        disabled: '無効 {disabled}',
      },
      groups: {
        title: 'Group ヘルス',
        description:
          '現在利用可能なキーだけを示し、特定の AccessKey、プロトコル、モデルのルート可否は保証しません。',
        empty: 'Group がありません。',
        disabled: '無効',
        configurationExcluded: '設定により除外',
        emptyKeys: 'キーなし',
        unavailable: '利用不可',
        attention: '要確認',
        limited: '容量制限',
        available: '利用可能',
        tableLabel: 'Group ヘルス一覧',
        columns: {
          group: 'Group',
          status: 'スケジュール',
          keyHealth: 'キー構成',
          exceptions: '現在の例外',
          actions: '操作',
        },
        keyHealthLabel:
          '全 {total} キー: 利用可能 {available}、クールダウン {cooldown}、ブラックリスト {blacklisted}、無効 {disabled}',
        cooldownCount: 'クールダウン {count}',
        blacklistedCount: 'ブラックリスト {count}',
        none: 'なし',
        viewLogsFor: '{name} のリクエストログを表示',
        viewGroup: 'Group {name} を表示',
        showAll: '全 {count} Group を表示',
        collapse: 'Group を折りたたむ',
      },
      problems: {
        title: '確認が必要なキー',
        description:
          '現在クールダウン中またはブラックリストのキーです。直近 {minutes} 分の統計を判断材料として表示します。',
        count: '{count} 件',
        empty: 'キーの状態は正常です',
        emptyDescription:
          '利用可能な {count} キーはすべて通常どおり参加しており、クールダウン中またはブラックリストのキーはありません。',
        emptyHint: '問題が発生したキーはスケジュールへの影響順にここへ表示されます。',
        inactiveTitle: 'ランタイムキーの例外はありません',
        inactiveDescription:
          'クールダウン中またはブラックリストのキーはありません。利用可否の設定は概要と Group ヘルスで確認してください。',
        noneForStatus: 'この状態のキーはありません。',
        cooldown: 'クールダウン中',
        blacklisted: 'ブラックリスト',
        keyId: 'Key #{id}',
        remaining: '残り {time}',
        keyMeta: '{group}',
        tableLabel: '確認が必要なキー一覧',
        columns: {
          identity: 'キー / Group',
          status: '状態',
          window: '現在の期間',
          recovery: '回復',
          actions: '操作',
        },
        window: '直近 {minutes} 分：例外 {problems} · 成功 {successes}',
        windowCompactPrefix: '{minutes}min:',
        windowLabel: '統計期間：直近 {minutes} 分',
        consecutive: '連続例外 {count}',
        cooldownRecovery: '約 {time} 後に回復',
        cooldownHint: 'クールダウン後に自動でスケジュールへ復帰',
        validationRecovery: '検証プローブ待ち',
        validationHint: 'プローブ成功後に自動回復',
        configurationRecovery: '検証設定が必要',
        configurationHint: '自動回復には検証モデルの設定が必要です',
        viewLogs: 'キー {key} のリクエストログを表示',
        viewGroup: 'Group {group} を表示',
      },
      details: {
        failureCount: '累計失敗',
        recentSuccessCount: '直近の成功',
        recentFailureCount: '直近の例外',
        consecutiveFailureCount: '連続例外',
        failureCategory: '失敗理由',
        statusCode: 'HTTP ステータス',
        statusUnavailable: 'HTTP ステータスなし',
        manualWeight: '手動ウェイト',
        autoWeight: '自動ウェイト',
        automatic: '自動',
      },
      recovery: {
        automatic: '自動回復',
        notAutomatic: '自動回復なし',
        cooldownExpiry: 'クールダウン期限',
        validationProbe: '検証プローブ',
        recoversAt: '自動回復時刻',
        runtimeDecides: 'プローブ時刻はランタイムが決定します',
        unknown: '回復方法はランタイムが決定します',
        seconds: '{seconds} 秒',
        minutesSeconds: '{minutes} 分 {seconds} 秒',
        hoursMinutes: '{hours} 時間 {minutes} 分',
        exact: '回復予定時刻: {time}',
      },
      failureCategories: {
        rate_limited: 'レート制限',
        model_unavailable: 'モデル利用不可',
        invalid_key: '無効なキー',
        upstream_host_error: 'アップストリームホストエラー',
        client_error: 'クライアントリクエストエラー',
        downstream_cancel: 'ダウンストリームでキャンセル',
        ambiguous: '不明確な失敗',
      },
      requestLog: {
        title: 'リクエストログ収集',
        description: '現在のプロセスの累積値で、再起動時にリセットされます。',
        normal: '収集は正常',
        abnormal: '収集に異常あり',
        retentionAbnormal: '保持処理に異常あり',
        queue: 'キュー',
        queueDescription: '現在 / 容量',
        processTotal: '現在のプロセス',
        persistedDescription: '現在のプロセスで永続化',
        enqueued: 'キュー投入済み',
        persisted: '永続化済み',
        droppedNotRunning: '破棄: 未実行',
        droppedQueueFull: '破棄: キュー満杯',
        droppedStopping: '破棄: 停止中',
        droppedPersistFailed: '破棄: 永続化失敗',
        droppedShutdown: '破棄: シャットダウン',
        droppedTotal: '破棄合計',
        writeFailures: '書き込み失敗',
        retentionFailures: '保持データ削除失敗',
        queueDepth: 'キュー深度',
        queueCapacity: 'キュー容量',
        lastWriteFailureAt: '直近の書き込み失敗',
        lastRetentionFailureAt: '直近の保持処理失敗',
      },
    },
    usage: {
      loading: '使用量レポートを読み込み中…',
      loadFailed: '使用量レポートを読み込めません。',
      stale: 'バックグラウンド更新に失敗したため、使用量データが古い可能性があります。',
      options: {
        groupsFailed:
          'Group 名を読み込めません。代わりに正の Group ID を入力できます。使用量レポートには影響しません。',
      },
      filters: {
        label: '使用量レポートのフィルター',
        title: '使用量とコストを絞り込む',
        description: 'Group とアップストリームモデルで現在の集計範囲を絞り込みます。',
        close: '使用量とコストのフィルターを閉じる',
        button: 'フィルター',
        range: '期間',
        range24h: '過去 24 時間',
        range30d: '過去 30 日',
        ranges: {
          '24h': '24 時間',
          '3d': '3 日',
          '7d': '7 日',
          '15d': '15 日',
          '30d': '30 日',
        },
        group: 'Group',
        groupHelp: '任意の完全一致 Group スコープです。',
        groupIdHelp: 'Group 名の読み込みに失敗しました。正の Group ID を入力できます。',
        anyGroup: 'すべての Group',
        model: 'アップストリームモデル',
        modelPlaceholder: '完全一致のモデル名',
        modelHelp: 'ルーティング後のアップストリームモデルを完全一致で絞り込みます。',
        deletedOrUnknown: '#{id} · 削除済みまたは不明',
        refresh: '更新',
        apply: '適用',
        reset: 'リセット',
        applied: '適用中のフィルター',
        anyModel: 'すべてのアップストリームモデル',
        dirty: 'フィルターの変更はまだ適用されていません。',
        observedAt: 'バックエンド観測時刻',
        refreshedAt: '最終更新時刻',
      },
      errors: {
        positiveId: '有効な正の Group ID を入力してください。',
        model: 'アップストリームモデルの前後に空白や制御文字を含めることはできません。',
      },
      scope: {
        title: '永続化された使用量と現在プロセスのパイプライン状態',
        description:
          '使用量、トークン、推定コストは選択した UTC 期間の永続化リクエストログに基づきます。破棄と書き込み失敗は現在プロセスの累積値で、レポートのフィルターには従いません。',
      },
      empty: {
        title: '一致する使用量データがありません',
        description:
          '期間を変更するか、Group とアップストリームモデルのフィルターをリセットしてください。',
      },
      kpi: {
        title: '使用量サマリー',
        description: 'バックエンド集計の観測時刻: {time}。',
        requests: 'リクエスト数',
        outcomes: '成功と失敗',
        outcomeCounts: '成功 {success} · 失敗 {failure}',
        successRate: '成功率 {rate}',
        totalTokens: '報告済みトークン合計',
        persistedWindow: '現在の集計期間',
        estimatedCost: '推定コスト',
      },
      tokens: {
        title: '報告済みトークン分類',
        description: 'アップストリームが返す provider-neutral の使用量フィールド別に表示します。',
        uncachedInput: 'キャッシュなし入力',
        cacheRead: 'キャッシュ読み取り',
        cacheWrite5m: 'キャッシュ書き込み（5 分）',
        cacheWrite1h: 'キャッシュ書き込み（1 時間）',
        cacheWriteUnknown: 'キャッシュ書き込み',
        output: '出力',
      },
      cost: {
        unknown: '不明',
        knownPlusUnknown: '{cost} + 不明',
        knownPlusUnknownShort: '判明分 + 不明分',
        known: '判明分の推定',
      },
      quality: {
        title: '使用量と永続化の品質',
        description: '不完全な使用量を単一の指標で隠さないよう、各件数を分けて表示します。',
        windowDescription:
          '永続化品質の件数は、選択期間内に永続化されたレコードだけを対象とします。',
        missing: '使用量なし',
        partial: '使用量が部分的',
        unpriced: 'コスト未設定',
        pricingPartial: '一部のみ価格設定',
        dropped: '破棄されたリクエスト',
        writeFailures: '書き込み失敗',
        overlap:
          '使用量なし、部分的、未設定の件数は重複する場合があります。リクエスト総数として加算しないでください。',
        partialExplanation:
          '使用量が部分的に報告された場合、既知の Token は合計に含まれ、未報告部分は不明のままです。',
        unpricedExplanation:
          '未設定リクエストの報告済み Token も合計に含まれ、コストは価格設定可能な部分のみ集計されます。',
        aggregation:
          'Token 合計には、完全または部分的に報告された既知の Token が含まれます。使用量なしと該当なしは除外されます。コストは価格設定可能な部分のみ集計され、不明部分が残る場合があります。',
        openPrices: 'モデル価格を確認',
      },
      process: {
        title: '現在プロセスの収集状態',
        description:
          '現在プロセスのカウンターはレポートフィルターに従わず、プロセス再起動時にリセットされます。',
        lastWriteFailure: '直近の書き込み失敗',
        never: '報告なし',
        clear: '現在プロセスではテレメトリ破棄や書き込み失敗が報告されていません。',
        droppedWarning:
          'テレメトリが破棄された場合、統計は正常に永続化されたリクエストだけを対象とします。',
        writeFailureWarning: '現在プロセスで RequestLog 永続化バッチの失敗が発生しました。',
        warning: '現在プロセスの収集異常: 破棄 {dropped}、書き込み失敗 {failures}。',
      },
      trend: {
        title: 'リクエストと失敗の推移',
        description: '疎な UTC バケットはそのまま表示し、欠落点を推測しません。',
        accessibleDescription: '返された UTC バケットごとのリクエスト数と失敗リクエスト数。',
        empty: 'この期間のリクエストバケットはありません。',
        hourly: '時間別',
        daily: '日別',
      },
      series: {
        title: 'UTC バケット',
        description: '選択した時間または日単位でバックエンドが集計した値です。',
        caption: 'UTC 時間バケット別の使用量集計',
        disclosure: '時間バケットの詳細を表示',
      },
      breakdown: {
        title: 'Group とアップストリームモデルの内訳',
        description: 'バックエンド順位の集計は最終 Group とアップストリームモデルを使用します。',
        caption: '最終 Group とアップストリームモデル別の使用量内訳',
        truncated: 'バックエンド上位 100 件の内訳のみ表示しています。',
        unattributedGroup: '未帰属 Group',
        unattributedMeta: 'Group への帰属なし',
        unknownModel: '不明なモデル',
        count: '全 {count} 件',
        truncatedShort: '結果は上限に達しました',
        orderLabel: '内訳の並び順',
        orderRequests: 'リクエスト数順',
        orderCost: '推定コスト順',
      },
      columns: {
        window: 'UTC 時間枠',
        group: '最終 Group',
        upstreamModel: 'アップストリームモデル',
        requests: 'リクエスト',
        success: '成功',
        failure: '失敗',
        totalTokens: 'トークン合計',
        estimatedCost: '推定コスト',
        quality: '品質件数',
        actions: '操作',
        qualityUsage: 'なし {missing} · 部分 {partial}',
        qualityPricing: '未設定 {unpriced} · 一部 {partial}',
        qualityCompact:
          'なし {missing} · 部分 {partial} · 未設定 {unpriced} · 部分価格 {pricingPartial}',
        qualityCounts: 'なし {missing} · 部分 {partial} · 未設定 {unpriced}',
      },
    },
    inspector: {
      title: 'ルート検査',
      description: '現在のランタイムに対する入力の候補 Group と Key を説明します。',
      boundary:
        '現在のランタイムのみを参照します。読み取り専用で、アップストリームへリクエストを送信せず、Token も消費しません。',
      protocols: {
        'openai-completions': 'OpenAI Completions',
        'openai-responses': 'OpenAI Responses',
        anthropic: 'Anthropic',
        gemini: 'Gemini',
      },
      form: {
        title: '模擬リクエスト条件',
        label: 'ルート検査の入力',
        protocol: 'プロトコル',
        model: 'クライアントモデル',
        optional: '任意',
        modelPlaceholder: 'クライアントモデルを入力',
        modelOptional: 'プロトコルのみで扱うリソースリクエストでは省略できます。',
        accessKey: 'AccessKey',
        accessKeyOption: '{name} · #{id} · {status}',
        missingAccessKeyOption: '存在しない AccessKey · #{id}',
        submit: '現在のルートを検査',
      },
      accessKeyStatus: {
        active: '有効',
        disabled: '無効',
      },
      options: {
        loading: 'AccessKey の選択肢を読み込み中…',
        failed: 'AccessKey の選択肢を読み込めません。',
      },
      errors: {
        protocol: '有効なプロトコルを選択してください。',
        model: '前後に空白のないモデル名を入力してください。',
        accessKey: '存在する AccessKey を選び直してください。',
        missingDeepLinkAccessKey:
          'リンクで指定された AccessKey #{id} は存在しません。選び直してください。',
        summary: 'ルート検査の入力が無効です。',
      },
      request: {
        loading: '現在のルート判断を読み取り中…',
        failed: '現在のルートを検査できません。',
      },
      empty: {
        title: '条件を入力して現在のルートを検査',
        description:
          '現在の設定スナップショットにある候補 Group、Key の状態、除外理由を説明します。',
      },
      result: {
        title: '現在のランタイム観測',
        routableTitle: '現在のリクエストはルート可能です',
        notRoutableTitle: '現在のリクエストはルートできません',
        routable: 'ルート可能',
        notRoutable: 'ルート不可',
        reasonLine: '理由：{reason}',
        observedAt: '観測時刻 {time}',
        revision: 'リビジョン {revision}',
        protocol: 'プロトコル',
        externalModel: 'クライアントモデル',
        modelNotSpecified: '未指定',
        accessKey: 'AccessKey',
        accessKeyStatus: 'AccessKey の状態',
        candidateGroups: '候補 Group',
        availableKeys: '現在利用可能な Key',
        reason: '理由',
        inputChanged: '入力が変更されました。以下は最後に送信した入力に対する観測結果です。',
        stale: '最新の検査に失敗しました。以下は表示された観測時刻の古い結果です。',
      },
      groups: {
        title: '候補 Group',
        description:
          '現在の候補 Group で利用可能な Key の有効ウェイト合計から算出し、固定のトラフィック比率を示すものではありません。',
        count: '{count} 件',
        tableLabel: '候補 Group のルート説明',
        completeEmpty:
          '候補 Group は返されませんでした。これは空状態ではなく、この入力に対する完全な現在ランタイムの説明です。',
        included: '候補に含まれる',
        excluded: '候補から除外',
        availableTotal: '利用可能 / 合計',
        currentTotal: '現在の合計',
        shareLabel: '{name} の有効ウェイト比率は {share}',
        viewGroup: 'Group を表示',
        columns: {
          group: 'Group',
          status: '状態',
          keys: 'Key',
          weight: '有効ウェイト',
          share: 'ウェイト比率',
          actions: '操作',
        },
      },
      keys: {
        title: 'Key 単位の説明',
        identity: 'Key #{id}',
        available: '利用可能',
        unavailable: '利用不可',
        summary: '利用可能 {available} · 利用不可 {unavailable}',
        tableLabel: '{name} の Key ルート説明',
        noneReturned: 'この Group には Key 候補が返されませんでした。',
        cooldownUntil: 'クールダウン期限',
        none: 'なし',
        columns: {
          key: 'Key',
          status: '状態',
          manual: '手動ウェイト',
          auto: '自動ウェイト',
          effective: '有効ウェイト',
          cooldown: 'クールダウン期限',
        },
      },
      weights: {
        manual: '生の手動ウェイト',
        auto: '生の自動ウェイト',
        effective: '生の有効ウェイト',
        null: 'null',
        none: '—',
        groupManual: 'Group 手動ウェイト {value}',
      },
      excluded: {
        title: '除外 Group',
        description:
          'Group 単位の除外結果のみを表示します。候補 Group を展開すると Key 単位の理由を確認できます。',
        empty: '現在の入力で除外された Group はありません。',
        tableLabel: '除外 Group のルート説明',
        reasonCode: '理由コード',
      },
      reasons: {
        none: '除外理由なし',
        access_key_disabled: 'AccessKey は無効です',
        protocol_filtered: 'AccessKey フィルターがこのプロトコルを除外しました',
        model_filtered: 'AccessKey フィルターがこのモデルを除外しました',
        model_required_by_filter: 'AccessKey のモデルフィルターによりモデルの指定が必要です',
        no_route_target: 'このモデルとプロトコルに一致するルート先がありません',
        group_disabled: 'Group は無効です',
        group_filtered: 'AccessKey フィルターがこの Group を除外しました',
        no_available_group: '現在ルート可能な候補 Group がありません',
        no_keys: 'Group にランタイム Key がありません',
        group_weight_zero: 'Group の有効ウェイトがゼロです',
        key_disabled: 'Key は無効です',
        key_blacklisted: 'Key はブラックリストに登録されています',
        key_cooldown: 'Key はクールダウン中です',
        key_weight_zero: 'Key の有効ウェイトがゼロです',
        no_available_key: 'Group に現在利用可能な Key がありません',
        unknown: '不明なルート理由',
      },
    },
    logs: {
      loading: 'リクエストログを読み込み中…',
      loadFailed: 'リクエストログを読み込めません。',
      loadMore: 'さらに読み込む',
      nextPageFailed: '次のページを読み込めません。現在のログは保持されています。',
      refresh: '更新',
      refreshFailed: 'リクエストログを更新できません。現在の全ページは保持されています。',
      stale: '最新の更新に失敗したため、最後に成功した結果を表示しています。',
      caption: 'リクエストログ',
      empty: {
        title: 'リクエストログはまだありません',
        description: 'クライアントリクエストを受信すると、秘匿化されたログが表示されます。',
        filteredTitle: '現在のフィルターに一致するログはありません',
        filteredDescription: 'フィルターを調整またはリセットして、再試行してください。',
      },
      options: {
        groupsFailed: 'Group フィルターの選択肢を読み込めません。ログ一覧には影響しません。',
        accessKeysFailed:
          'AccessKey フィルターの選択肢を読み込めません。ログ一覧には影響しません。',
        partialFailed: '一部のフィルター選択肢を読み込めませんが、ログ検索は使用できます。',
      },
      filters: {
        label: 'リクエストログのフィルター',
        timeRange: '時間範囲',
        from: '開始時刻',
        to: '終了時刻',
        quickRanges: 'クイック時間範囲',
        quick: {
          '15m': '15m',
          '1h': '1h',
          '24h': '24h',
          '3d': '3d',
          '7d': '7d',
          '15d': '15d',
        },
        any: 'すべて',
        group: 'Group',
        groupHelp: 'いずれかの試行で使用された Group に一致します。',
        anyGroup: 'すべての Group',
        model: 'クライアントモデル',
        clientModel: 'クライアントモデル',
        upstreamModel: 'アップストリームモデル',
        accessKey: 'AccessKey',
        anyAccessKey: 'すべての AccessKey',
        protocol: 'プロトコル',
        anyProtocol: 'すべてのプロトコル',
        stream: 'ストリーミング',
        status: '状態',
        anyStatus: 'すべての状態',
        requestId: 'リクエスト ID',
        finalStatusCode: '最終 HTTP ステータス',
        upstreamKey: 'アップストリーム Key ID',
        attemptStatusCode: '試行のステータス',
        failureCategory: '試行の失敗分類',
        errorCode: '試行のエラーコード',
        retryStateLabel: '再試行状態',
        retryMin: '最小再試行回数',
        retryMax: '最大再試行回数',
        usageStateLabel: '使用量状態',
        costStateLabel: 'コスト状態',
        completenessLabel: '価格計算の完全性',
        cachePresent: 'キャッシュ Token あり',
        more: 'その他のフィルター',
        apply: '適用',
        reset: 'リセット',
        dirty: 'フィルターの下書きはまだ適用されていません',
        applied: '適用中のフィルター',
        noneApplied: 'フィルターなし',
        appliedFrom: '開始 {value}',
        appliedTo: '終了 {value}',
        appliedGroup: 'Group {value}',
        appliedModel: 'クライアントモデル {value}',
        appliedClientModel: 'クライアントモデル {value}',
        appliedUpstreamModel: 'アップストリームモデル {value}',
        appliedAccessKey: 'AccessKey {value}',
        appliedStatus: '状態 {value}',
        appliedUsageState: '使用量 {value}',
        appliedCostState: 'コスト {value}',
        appliedCompleteness: '料金 {value}',
        appliedRequestId: 'リクエスト ID {value}',
        timezone: 'ローカルタイムゾーン',
        lastRefreshed: '最終成功更新',
        remove: 'フィルター {value} を削除',
        advancedTitle: 'その他のフィルター',
        advancedDescription: 'リクエスト、再試行、最終結果、数値範囲で絞り込みます。',
        closeAdvanced: 'その他のフィルターを閉じる',
        sections: {
          request: 'リクエスト',
          attempt: '再試行',
          result: '最終結果',
          ranges: '数値範囲',
        },
        retryState: { retried: '再試行あり', not_retried: '再試行なし' },
        usageState: {
          complete: '完全',
          partial: '部分的',
          missing: '欠落',
          not_applicable: '該当なし',
        },
        costState: {
          priced: '計算済み',
          unpriced: '未計算',
          not_applicable: '該当なし',
        },
        completeness: {
          complete: '完全',
          partial: '部分的',
          unavailable: '利用不可',
          not_applicable: '該当なし',
        },
        rangeFields: {
          retry_count_min: '最小再試行回数',
          retry_count_max: '最大再試行回数',
          first_response_min_ms: '最短初回応答（ms）',
          first_response_max_ms: '最長初回応答（ms）',
          duration_min_ms: '最短所要時間（ms）',
          duration_max_ms: '最長所要時間（ms）',
          input_tokens_min: '最小入力 Token',
          input_tokens_max: '最大入力 Token',
          output_tokens_min: '最小出力 Token',
          output_tokens_max: '最大出力 Token',
          cost_min_usd: '最小コスト（USD）',
          cost_max_usd: '最大コスト（USD）',
        },
      },
      errors: {
        dateTime: '有効なローカル日時を入力してください。',
        range: '終了時刻は開始時刻より後である必要があります。',
        positiveId: '有効な正の ID を選択してください。',
        model: 'クライアントモデルの前後に空白や制御文字は使用できません。',
        status: '有効な状態を選択してください。',
        requestId: '正規の小文字 UUIDv4 を入力してください。',
        nonNegativeInteger: '0 以上の整数を入力してください。',
        numericRange: '最大値は最小値以上である必要があります。',
        text: '前後の空白や制御文字は使用できません。',
        usd: '小数点以下 9 桁までの 0 以上の USD 金額を入力してください。',
      },
      accessKey: {
        named: '{name} · #{id}',
        deleted: '削除済み · #{id}',
      },
      none: 'なし',
      yes: 'はい',
      no: 'いいえ',
      attemptCount: '{count} 回の試行',
      details: '詳細を表示',
      failureCategory: {
        ok: '正常',
        rate_limited: 'レート制限',
        model_unavailable: 'モデル利用不可',
        invalid_key: '無効なキー',
        upstream_host_error: 'アップストリームホストエラー',
        client_error: 'クライアントエラー',
        downstream_cancel: 'ダウンストリームによるキャンセル',
        ambiguous: '原因不明',
        unknown: '不明な分類',
      },
      action: {
        terminate: '終了',
        retry: '再試行',
        cooldown_key: 'Key をクールダウン',
        fail_key: 'Key を失敗扱い',
        skip_group: 'Group をスキップ',
        unknown: '不明なアクション',
      },
      usage: {
        missing: '使用量なし',
        not_applicable: '該当なし',
      },
      cost: {
        knownSubtotal: '既知 {cost} + 不明',
        unpriced: 'コスト不明',
        not_applicable: '該当なし',
      },
      drawer: {
        title: 'リクエストログ詳細',
        description: '最終結果と秘匿化されたアップストリーム試行チェーンを確認します。',
        close: 'リクエストログ詳細を閉じる',
        loading: 'リクエストログ詳細を読み込み中…',
        loadFailed: 'リクエストログ詳細を読み込めません。',
        summary: '最終結果',
        requestId: 'リクエスト ID',
        copyRequestId: 'リクエスト ID をコピー',
        completedAt: '完了時刻',
        status: '状態',
        protocol: 'プロトコル',
        accessKey: 'AccessKey',
        clientModel: 'クライアントモデル',
        upstreamModel: 'アップストリームモデル',
        modelNotSpecified: '未指定',
        statusCode: 'ステータスコード',
        duration: '所要時間',
        firstResponse: '初回応答',
        outputRate: '出力速度',
        attemptCount: '試行回数',
        route: 'リクエストと最終ルート',
        affinity: 'アフィニティ一致',
        errorCode: 'エラーコード',
        errorSummary: '秘匿化されたエラー概要',
        openInspector: '現在の状態で確認（履歴の再現ではありません）',
        attempts: '試行チェーン',
        noAttempts: 'このリクエストにはアップストリーム試行がありません。',
        attempt: '試行 #{sequence}',
        committed: '確定済み',
        notCommitted: '未確定',
        group: '履歴 Group',
        upstreamKey: '履歴アップストリーム Key',
        failureCategory: '失敗分類',
        action: 'アクション',
        willRetry: '再試行予定',
        usage: {
          title: '使用量と推定コスト',
          description:
            '最終的に永続化された使用量フィールドです。コストは常に推定値で、不明は無料を意味しません。',
          finalGroup: '最終 Group',
          groupId: 'Group #{id}',
          usageStateLabel: '使用量状態',
          costStateLabel: 'コスト状態',
          estimatedCost: '推定コスト',
          unknown: '不明',
          notApplicable: '該当なし',
          openPrices: 'このアップストリームモデルの価格を確認',
          state: {
            complete: '完全な使用量',
            partial: '部分的な使用量',
            missing: '使用量なし',
            not_applicable: '使用量は該当なし',
          },
          costState: {
            priced: '推定コストあり',
            partialPriced: '報告済みトークンからの推定コスト',
            unpriced: '推定コスト不明',
            not_applicable: 'コストは該当なし',
          },
          aggregation: {
            completePriced: '既定のトークンと推定コスト集計に含まれます。',
            completeUnpriced: '報告済み Token は集計され、コストは不明のままです。',
            partialPriced: '報告済み Token と既知コストはそれぞれ集計され、残りは不明です。',
            partialUnpriced: '報告済み Token は集計され、コストは不明のままです。',
            missing: '使用量がないため、Token とコストは集計に含まれません。',
            notApplicable:
              '使用量とコストが該当しないため、既定のトークンと推定コスト集計から除外されます。',
          },
          tokens: {
            uncachedInput: 'キャッシュなし入力',
            cacheRead: 'キャッシュ読み取り',
            cacheWrite5m: 'キャッシュ書き込み（5 分）',
            cacheWrite1h: 'キャッシュ書き込み（1 時間）',
            output: '出力',
          },
        },
      },
      columns: {
        time: '時刻',
        modelProtocol: 'モデル / プロトコル',
        response: '応答',
        cost: 'コスト',
        tokens: 'Tokens',
        completedAt: '完了時刻',
        requestId: 'リクエスト ID',
        route: 'ルート',
        accessKey: 'AccessKey',
        result: '最終状態',
        timing: '初回 / 合計',
        actions: '操作',
      },
      status: {
        success: '成功',
        error: 'エラー',
        incomplete: '未完了',
        canceled: 'キャンセル済み',
      },
      response: {
        normal: '正常',
        streamError: 'ストリームエラー',
        errorWithCode: 'エラー {code}',
      },
      modelMapping: 'クライアントモデル {client} → アップストリームモデル {upstream}',
      modelMappingLabel: 'モデルマッピングを表示',
      tokens: {
        input: '入力',
        output: '出力',
        cacheRead: 'キャッシュ読み取り',
        cacheWrite5m: 'キャッシュ書き込み 5m',
        cacheWrite1h: 'キャッシュ書き込み 1h',
        cacheWrite: 'キャッシュ書き込み',
        cacheDetails: 'キャッシュ Token の詳細を表示',
        partial: 'Token 使用量が部分的です',
      },
      receipt: {
        formula: 'コスト計算',
        input: '入力',
        output: '出力',
      },
    },
  },
} as const
